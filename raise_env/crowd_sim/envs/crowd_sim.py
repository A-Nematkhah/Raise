...
class CrowdSim(gym.Env):
    ...
    """
    def __init__(self, reward_fn: Optional[RewardFunction] = None):
        """
        ...
        reward_fn: optional pluggable scalar reward. Termination / episode_info
        stay env-owned and never depend on reward_fn. Default (None) installs
        LegacyReward at configure() time so trained_models stay reproducible.
        """
        ...
        # Pluggable scalar reward (termination stays env-owned).
    ...
    def configure(self, config):
        ...
        # Install LegacyReward by default so scalar reward matches the pre-refactor formula.
        if self._reward_fn_override is None:
            self.reward_fn = None  # finalized after robot is set (kinematics known)
        else:
            self.reward_fn = self._reward_fn_override
        ...
        if self._reward_fn_override is None:
            self.reward_fn = LegacyReward.from_crowd_sim(self)
        else:
            self.reward_fn = self._reward_fn_override

    def set_reward_fn(self, reward_fn: Optional[RewardFunction] = None) -> None:
        """Inject a custom RewardFunction, or restore LegacyReward when None."""
        self._reward_fn_override = reward_fn
    ...
    def calc_reward(self, action):
        ...
        timeout = self.global_time >= self.time_limit - 1

        # Termination / episode_info: env-owned, independent of reward_fn.
        if timeout:
            done = True
        ...
        # Scalar reward: pluggable (default LegacyReward = original formula).
        ...
        reward = self.reward_fn.compute(state)

        # Keep env.potential in sync when using LegacyReward (other code may read it).
        if isinstance(self.reward_fn, LegacyReward) and self.reward_fn._potential is not None:
            self.potential = self.reward_fn._potential
