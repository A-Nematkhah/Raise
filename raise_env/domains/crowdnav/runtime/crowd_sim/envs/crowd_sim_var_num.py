...
class CrowdSimVarNum(CrowdSim):
    ...
    def calc_reward(self, action, danger_zone='circle'):
        ...
        # use danger_zone to determine the condition for Danger (episode_info only)
        ...
        # LegacyReward discomfort branch keys off dmin < discomfort_dist.
        # When danger_zone='future' marks Danger without close dmin, match the
        # original scalar by temporarily treating danger_cond like discomfort.
        # Only apply this for LegacyReward so custom candidates see true dmin.
        reward_dmin = dmin
        if (
            isinstance(self.reward_fn, LegacyReward)
            and danger_cond
            and not (dmin < self.discomfort_dist)
            and not (timeout or collision or reaching_goal)
        ):
            reward_dmin = min(dmin, self.discomfort_dist - 1e-8)
