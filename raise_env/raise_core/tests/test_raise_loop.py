...
from raise_core.raise_loop.epoch import select_to_label, unique_by_code
from raise_core.raise_loop.logging_io import read_epochs
...
def test_al_inside_epoch_hard_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "raise_core.raise_loop.epoch.score_queries",
        _fake_score_queries,
    )
    to_label, gate, al = select_to_label(
        pop,
